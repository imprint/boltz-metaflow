...
class Boltz1AntibodyBenchmarkFlow(FlowSpec):
    ...
    def expand_tasks(self, base_tasks):
        ...
        for task_id, task in base_tasks.items():
            antigen_sequence = task["input"]["sequences"][0]["protein"]["sequence"]
            for residue in range(1, len(antigen_sequence), 10):
                for (chain_id, chain_name) in [
                    ...
                    task["info"]["antigen_pocket_residue"] = residue
                    task["info"]["antigen_pocket_binder"] = chain_name
                    ...
                    new_task_dict[f"{task_id}_{len(new_task_dict)}"] = task
